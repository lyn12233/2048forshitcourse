...
class myframe(wx.Frame):
    def __init__(
        ...
        self.tmspan = 0.0  # time span ,also used time
    ...
    def init_ui(self):
        self.p1 = wx.Panel(self)  # p1 for game page. now multipage isn't implemented
        ...
        self.bmps = [
            wx.Bitmap(
                wx.Image(f"./img/bmp/tile_{i}.bmp", wx.BITMAP_TYPE_BMP).Scale(
                    *GUI_TILE_SIZE
                )
            )
            for i in range(12)
    ...
    def update_te(self):
        ...
        if self.state == PENDING:
            ...
            inf.append(f"user: {tr(self.user)}")
            inf.append(
                f"used time: {tr(self.tmspan)}" + "(CHEATED)" * (self.state == CHEATING)
            )
        else:
